...
window.onload = function(){
    ...
    var socket = new ReconnectingWebsocket('ws://' + getAllUrlParams().websocketserver + ':' + getAllUrlParams().websockport);
    ...
    function extractControls(data){
        ...
        switch(config.controllerType){
            case'GCN':
                ret.button.START = Boolean(~~data[3]);
                ret.button.A = Boolean(~~data[7]);
                ret.button.B = Boolean(~~data[6]);
                ret.button.X = Boolean(~~data[5]);
                ret.button.Y = Boolean(~~data[4]);
                ret.button.Z = Boolean(~~data[11]);
                ret.button.L = Boolean(~~data[9]);
                ret.button.R = Boolean(~~data[10]);
                // Todo analog shennangins

                ret.analog.joyStickX = (getMultiByte(data, 16) - 128) / 128;
                ret.analog.joyStickY = (getMultiByte(data, 16 + 8) - 128) / 128;
                ret.analog.cStickX = (getMultiByte(data, 16 + 16) - 128) / 128;
                ret.analog.cStickY = (getMultiByte(data, 16 + 24) - 128) / 128;
                ret.analog.lTrig = getMultiByte(data, 16 + 32) / 256;
                ret.analog.rTrig = getMultiByte(data, 16 + 40) / 256;
                break;

            case'N64':
                ret.button.START = Boolean(~~data[3]);
                ret.button.A = Boolean(~~data[0]);
                ret.button.B = Boolean(~~data[1]);
                ret.button.Z = Boolean(~~data[2]);
                ret.button.L = Boolean(~~data[10]);
                ret.button.R = Boolean(~~data[11]);
                ret.button.up = Boolean(~~data[4]);
                ret.button.down = Boolean(~~data[5]);
                ret.button.left = Boolean(~~data[6]);
                ret.button.right = Boolean(~~data[7]);
                ret.button.Cup = Boolean(~~data[12]);
                ret.button.Cdown = Boolean(~~data[13]);
                ret.button.Cleft = Boolean(~~data[14]);
                ret.button.Cright = Boolean(~~data[15]);

                ret.analog.joyStickX = (~~getMultiByte(data, 16));
                ret.analog.joyStickY = (~~getMultiByte(data, 16 + 8));
                if(ret.analog.joyStickX > 127){
                    ret.analog.joyStickX -= 256;
                }
                if(ret.analog.joyStickY > 127){
                    ret.analog.joyStickY -= 256;
                }
                ret.analog.joyStickX /= 128;
                ret.analog.joyStickY /= 128;
                break;
            default:
                console.log('No Controller type');
        }
